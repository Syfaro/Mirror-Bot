...
class ImgurUpload:
	...
	def make_request(self, method, verb='GET', params=None):
		...
		if verb == 'GET':
			return requests.get(self.endpoint + method, data=params, headers=headers).json()
		else:
			return requests.post(self.endpoint + method, data=params, headers=headers).json()

	def upload_image_by_url(self, url, title=None, description=None):
		"""
		Uploads an image to imgur using a url, optionally with a `title` and `description`.
		"""
		...
		params = {'image': encoded, 'type': 'base64', 'title': title, 'description': description}

		return self.make_request('image', 'POST', params)

	def upload_image_by_file(self, path, title=None, description=None):
		"""
		Uploads an image to imgur using a file, optionally with a `title` and `description`.
		"""
